...
<head>
<meta charset="UTF-8">
<title>Danh sách tin tức</title>
<style type="text/css">
	* {
	    margin: 0;
	    padding: 0;
...
<body>
	<div class="container">
		...
		<div class="content" style="padding: 20px; background-color: #f9f9f9;">
		    <form action="${pageContext.request.contextPath}/list" method="GET" style="margin-bottom: 20px;">
		        <label for="danhMuc" style="font-weight: bold; margin-right: 10px;">Chọn danh mục</label>
		        <select name="danhMucId" style="padding: 5px; border-radius: 5px; border: 1px solid #ddd;">
		            <option value="">
		                <c:choose>
		                    <c:when test="${empty danhMucSelected}">Tất cả</c:when>
		                    <c:otherwise>${danhMucSelected}</c:otherwise>
		                </c:choose>
		            </option>
		        ...
		        <input type="submit" value="Lọc" style="padding: 5px 10px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">
		    </form>
		    <h1 style="margin-bottom: 20px;">Danh sách danh mục</h1>
		    <table border="1" style="width: 100%; border-collapse: collapse;">
		        ...
		        <c:forEach var="tinTuc" items="${dsTinTuc}">
		            <tr>
